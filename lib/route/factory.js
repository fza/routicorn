...
_.extend(RouteFactory.prototype, {
  ...
  _createRoute: function (routeName, routeConfig, basePath) {
    ...
    if (routeConfig.tags) {
      if (!_.isArray(routeConfig.tags)) {
        routeConfig.tags = [routeConfig.tags];
      }

      routeOptions.tags = routeConfig.tags.filter(function (tag) {
        return _.isString(tag);
      });
    }

    if (_.isString(routeConfig.tag)) {
      routeOptions.tags.push(routeConfig.tag);
    }
