...
function ActionRoute(routeName, pattern, options) {
  ...
  self._setActionable();
  self._handleRequestParamsOnAction = false;
...
_.extend(ActionRoute.prototype, {
  ...
  mount: function (expressRouter) {
    ...
    var path = this._handleRequestParamsOnAction ? self.pattern : '/';
    _.each(this._actions, function (actionHandler) {
      ...
      expressRouter[actionHandler[0]](path, actionHandler[1]);
    });
  ...
  _invokeAction: function (fn, req, res, next) {
    ...
    if (self._handleRequestParamsOnAction) {
      return this._handleRequestParams(req, res, function (err) {
        if (err) {
          return next(err);
        }
