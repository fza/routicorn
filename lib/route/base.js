 ...
 *
 * Events:
 * - `beforeMount`: Emitted just before mounting sub-routes or actions. Arguments: (none)
 * - `mount`: Emitted after mounting sub-routes or actions. Arguments: (none)
 *
 * @constructor
 * @abstract
 *
...
_.extend(BaseRoute.prototype, {
  ...
  /**
   * Add middleware to the shimmed express router (express compatible API)
   *
  ...
  use: function (fn) {
    ...
    if (fns.length === 0) {
      throw new TypeError('BaseRoute#use() requires middleware functions');
    }

    debug('Register %d middleware before route: %s', fns.length, this.name);

    [].push.apply(this._middleware, fns.map(function (middlewareFn) {
      return [path, middlewareFn];
    }));
  },
   ...
   *   most route to include in the pattern with higher order routes being excluded. If this is a
   *   boolean, the returned value will be an array of path segments and corresponding routes with
   *   the first element being the complete path and the second item being the query string, like:
   *   `['/foo/bar/baz', '?abc=123', ['/foo', routeInstance], …]`. The default is to return a
   *   single string with all segments from the current to the root route.
   *
   * @returns {*}
   */
  generatePath: function (params, query, checkRequirements, mod) {
    var self = this;
