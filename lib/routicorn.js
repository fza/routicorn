...
_.extend(Routicorn.prototype, {
   ...
   *   configs.
   * @param {boolean} [mountRoutes=false] Whether to mount the loaded routes onto the shimmed root
   *   express router.
  ...
  loadRoutes: function (source, mountRoutes, controllerFactory) {
    ...
    var routeFactory = new RouteFactory(self, {
    ...
    routeFactory.on('routeCreated', onRouteCreated);

    var routes;
    if (_.isString(source)) {
      routes = routeFactory.createRoutesFromYmlFile(source, controllerFactory);
    } else {
      routes = routeFactory.createRoutesFromConfigs(source, controllerFactory);
    }
    ...
    routeFactory.off('routeCreated', onRouteCreated);
  },
